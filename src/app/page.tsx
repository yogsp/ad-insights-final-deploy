...
const TabButton = ({ label, isActive, onClick, icon }) => (
  <button
    onClick={onClick}
    className={`flex items-center justify-center px-4 sm:px-6 py-3 font-semibold rounded-t-lg transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 focus:ring-offset-slate-50 ${
      isActive
...
const AdAnalyzer = () => {
    ...
    // Initialize API key from localStorage or an empty string
    ...
    const generatePrompt = (selectedPlatform) => {
        let platformContext = '';
        ...
        return `As an expert marketing analyst, analyze the provided ad image. ${platformContext} Your response must be a valid JSON object.

        The JSON response must strictly follow this schema:
        {
            "overallScore": 85,
            "oneLiner": "A single, concise sentence summarizing the ad's core message and appeal.",
            "strengths": ["An array of 2-4 strings detailing positive elements relevant to the platform."],
            "improvements": ["An array of 2-4 strings with actionable suggestions relevant to the platform."],
            "analysis": {
                "colorPalette": { "vibe": "string", "recommendation": "string" },
                "subjectAndComposition": { "focus": "string", "type": "string", "recommendation": "string" },
                "typographyAndText": { "clarity": "string", "hierarchy": "string", "recommendation": "string" },
                "brandIntegration": { "logoVisibility": "string", "recommendation": "string" },
                "callToAction": { "clarity": "string", "recommendation": "string" }
            }
        }
        `;
    };
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200/80 space-y-4">
                    ...
                    <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <PlatformButton 
                                ...
                                isActive={platform === 'social'} 
                                onClick={() => setPlatform('social')}
                            />
                             <PlatformButton 
                                label="Display/Banner" 
                                icon={<Monitor className="w-5 h-5 mr-2"/>}
                                isActive={platform === 'display'} 
                                onClick={() => setPlatform('display')}
                            />
                        </div>
                    </div>

                    <div 
                        className={`relative border-2 border-dashed rounded-lg p-6 text-center transition-colors duration-200 ${isDragging ? 'border-indigo-500 bg-indigo-50' : 'border-slate-300 hover:border-indigo-400'}`}
                        onDrop={handleDrop}
                    ...
                    <div className="mt-2">
                        ...
                        <div className="relative group">
                            ...
                            <button 
                                ...
                                className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-white"
                ...
                <div className="mt-2">
                    <button
                        ...
                        className="w-full bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow-md"
                    >
                        {isLoading ? (
                            <>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-200/80">
                <h2 className="text-2xl font-bold text-slate-800 mb-4">3. Performance Analysis</h2>
                 {!analysisResult && !isLoading && (
                    <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 bg-slate-50 rounded-lg p-8">
                        <FileImage className="w-16 h-16 text-slate-400 mb-4" />
                        <h3 className="text-xl font-semibold text-slate-700">Awaiting Analysis</h3>
                        <p className="max-w-xs mt-1">Upload an image and click "Analyze" to see your ad's performance prediction.</p>
                    </div>
                )}
                {isLoading && (
                     <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 p-8">
                        ...
                        <style>{`.loader{border-top-color:#4f46e5;animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}`}</style>
                        <h3 className="text-xl font-semibold text-slate-700 animate-pulse">Running AI Analysis...</h3>
                        <p className="max-w-xs mt-1">This may take a few moments.</p>
                    </div>
                )}
                {analysisResult && (
                    <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FeedbackCard title="Key Strengths" items={analysisResult.strengths} icon={<CheckCircle className="text-green-500"/>} />
...
const PlatformButton = ({ label, icon, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={`flex items-center justify-center w-full px-3 py-2 text-sm font-semibold border rounded-md transition-colors ${
            isActive 
